<#
TestFileAge.ps1
Use this to test files to make sure they are getting updated.
Results are passed to a command line of another script that gets launched, either every pass or only when the status changes.

See Readme.md for more information.

Usage:
TestFileAge.ps1 -mode once
TestFileAge.ps1 -mode loop
#>
Param( 
...
if ($null -eq $settings.test_every_n_mins)            {$settings.test_every_n_mins            = "10"; $settings_updated = $true}
...
if ($null -eq $settings.launch_param4_value) {$settings.launch_param4_value = "FileAge test %TestName% From $($scriptBase)"; $settings_updated = $true}
#
...
Write-Host "          test_every_n_mins: " -NoNewline; Write-Host $settings.test_every_n_mins -NoNewline -ForegroundColor Green; Write-Host " [Minutes between tests, Use 0 (or mode -once) to run and exit, which is suitable when running via a scheduled task]"
...
if (-not (Test-Path $csvTests)) {
    ...
    $lines=@()
    $lines += "TestName,Folder,FolderSubdirsToo,Filespec,MaxAgeMins,Status-Current,Date-Current,Status-Prior,Date-Prior,Disabled,Silenced"
    $lines += "Log Files,c:\test,False,*.txt,1440,OK,4/8/2025 9:49,,,,"
...
$test_every_n_mins = [int]$settings.test_every_n_mins # convert to number
Do
{ # run tests
    ...
    { # each test
        ...
        $msg1 = "$($test_i): $($TestName) <$($Test.Folder)\$($Test.Filespec)> (Newer than $($Test.MaxAgeMins) mins) ... "
        Write-Host $msg1 -NoNewline
        $TestFolder = $Test.Folder
        if ($TestFolder -eq "") {Write-Warning "Folder column is empty (it will be disabled)"; $Test.disabled -eq 'true'}
        ...
        { # Test Enabled
            # Get test settings
            $TestFolder = $Test.Folder
            $TestFileSpec = $Test.Filespec
            $TestMaxAgeInMins = [int]$Test.MaxAgeMins
            $FolderSubdirsToo = [bool]$Test.FolderSubdirsToo
            # Validate inputs
            if ($TestFileSpec -eq "") {$TestFileSpec="*.*"}
            if ($TestMaxAgeInMins -le 0) {$TestMaxAgeInMins=1440} # default to 1 day
            #region: Do the file age test
            if (-not (Test-Path -LiteralPath $TestFolder -PathType Container)) {
                $strStatus = "ERR: No folder found"
            } 
            else {
                if ($FolderSubdirsToo) {
                    $files = Get-ChildItem -LiteralPath $TestFolder -Filter $TestFileSpec -File -Recurse | Sort-Object LastWriteTime -Descending
                }
                else {
                    $files = Get-ChildItem -LiteralPath $TestFolder -Filter $TestFileSpec -File | Sort-Object LastWriteTime -Descending
                }
                if (-not $files) {
                    $strStatus = "ERR: No file found"
                }
                else {
                    $newest = $files[0]
                    $ageInMins = ((Get-Date) - $newest.LastWriteTime).TotalMinutes
                    if ($ageInMins -gt $TestMaxAgeInMins) {
                        $strStatus = "ERR: Newest file is $($newest.FullName) ($($newest.LastWriteTime)) and older than $TestMaxAgeInMins mins threshold"
                    } # file too old
                    else {
                        $strStatus = "OK: Newest file is $($newest.FullName) ($($newest.LastWriteTime)) within $TestMaxAgeInMins mins"
                    } # file is new enough
                } # files found
            } # folder exists
            #endregion: Do the file age test
            # interpret status
            Write-Host "[$($strStatus)] " -NoNewline
            if ($strStatus.StartsWith("ERR:")) {
                $TestStatus = "Fail"
        ...
        { # launch indicated
            ...
            Foreach ($i in 1..9) {
                ...
                if ($pnam -ne ""){
                    ...
                    # replace vars
                    $pval = $pval.Replace("%computername%",$env:COMPUTERNAME)
                    $pval = $pval.Replace("%TestFolder%",$TestFolder)
                    $pval = $pval.Replace("%TestName%"  ,$TestName)
                    $pval = $pval.Replace("%TestResult%",$TestResult)
                    # custom
                    if ([bool]$Test.FolderSubdirsToo) 
                        {$TestDescription = "$($Test.Folder)\*\$($Test.Filespec) age less than $($Test.MaxAgeMins) mins [$($strStatus )]"} 
                        else
                        {$TestDescription = "$($Test.Folder)\$($Test.Filespec) age less than $($Test.MaxAgeMins) mins [$($strStatus )]"} 
                    $pval = $pval.Replace("%TestDescription%",$TestDescription)
                    # boolean
    ...
    # save date
    if (($test_every_n_mins -gt 0) -and ($mode -eq 'loop'))
    { # loop tests
        $next_test_time = (Get-Date).AddMinutes($test_every_n_mins)
        Write-Host "Next test in $(($test_every_n_mins)) minutes (at $($next_test_time.ToString("yyyy-MM-dd HH:mm")) <Press Enter to test now>..." -NoNewline
